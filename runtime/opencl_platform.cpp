    ...
    : Platform(runtime)
{
    ...
    for (size_t i=0; i<num_platforms; ++i) {
        ...
        for (size_t j=0; j<num_devices; ++j) {
            ...
            #endif
            ILOG("      Device SPIR Support: % %", map_bool(has_spir), spir_version);
            ...
            ILOG("      Device Host Unified Memory: %", map_bool(has_unified));
            checkErr(err, "clGetDeviceInfo()");
            ...
            #endif
            if (!dd.queue) {
                dd.queue = clCreateCommandQueue(dd.ctx, dd.dev, CL_QUEUE_PROFILING_ENABLE, &err);
