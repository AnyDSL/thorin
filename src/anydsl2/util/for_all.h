...
#define for_all_t_x(var, what, step) \
    if (bool LNAME(break) = false) {} else \
        for (typename anydsl2::SelIter<BOOST_TYPEOF((what)), void (int var)>::iter LNAME(iter) = anydsl2::begin((what)), LNAME(end) = anydsl2::end((what)); !LNAME(break) && LNAME(iter) != LNAME(end); LNAME(break) ? (void)0 : (void)((step), ++LNAME(iter))) \
