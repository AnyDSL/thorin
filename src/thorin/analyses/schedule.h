...
public:
    ...
    const std::vector<const PrimOp*>& operator [] (Lambda* lambda) const { return blocks_[scope().sid(lambda)]; }
...
private:
    std::vector<const PrimOp*>& lookup(Lambda* lambda) { return blocks_[scope().sid(lambda)]; }
