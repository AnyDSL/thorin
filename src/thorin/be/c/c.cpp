...
void CCodeGen::emit_epilogue(Continuation* cont) {
    ...
    if (cont->callee() == entry_->ret_param()) { // return
    ...
    } else if (auto callee = cont->callee()->isa_nom<Continuation>(); callee && callee->intrinsic() == Intrinsic::Match) {
        bb.tail.fmt("switch ({}) {{\t\n", emit(cont->arg(0)));
    ...
    } else if (auto callee = cont->callee()->isa_nom<Continuation>(); callee && callee->is_basicblock()) { // ordinary jump
    ...
    } else if (auto callee = cont->callee()->isa_nom<Continuation>(); callee && callee->is_intrinsic()) {
        if (callee->intrinsic() == Intrinsic::Reserve) {
            ...
            auto ret_cont = cont->arg(2)->as_nom<Continuation>();
            ...
            auto body = cont->arg(4)->as_nom<Continuation>();
    ...
    } else if (auto callee = cont->callee()->isa_nom<Continuation>()) { // function/closure call
