...
inline bool is_floating(const Type* type, bool is_double = false) {
    if (auto primtype = type->isa<PrimType>()) {
        switch (primtype->primtype_tag()) {
        case PrimType_pf16:
        case PrimType_qf16:  THORIN_UNREACHABLE;

        case PrimType_pf32:
        case PrimType_qf32: return !is_double;
        case PrimType_pf64:
        case PrimType_qf64: return is_double;
...
// Detect struct { float x,y } and the like
inline bool is_complex(const Type* type) {
    if (auto struct_type = type->isa<StructType>()) {
        if (struct_type->num_ops() != 2)
            return false;
        auto fst = struct_type->op(0);
        auto snd = struct_type->op(1);
        if (is_floating(fst) && is_floating(snd))
            return true;
        return false;
    }
    return false;
...
inline bool is_double_complex(const Type* type) {
    if (auto struct_type = type->isa<StructType>()) {
        if (struct_type->num_ops() != 2)
            return false;
        auto fst = struct_type->op(0);
        auto snd = struct_type->op(1);
        if (is_floating(fst, true) && is_floating(snd, true))
            return true;
        return false;
    }
    return false;
...
void F95CodeGen::emit_f95_int() {
    ...
    for (auto continuation : world().continuations()) {
        ...
        auto fn_name = continuation->name();
        if(fn_name.str().back() != '_')
          assert(false && "Functions must end with an underscore to be compatible with Fortran");
        fn_name = fn_name.str().substr(0, fn_name.str().size() - 1);

        // emit return type declaration
        emit_aggop_decl(ret_type);
        emit_type(func_decls_, ret_type) << " " << fn_name << "_ret" << endl;
        emit_type(func_decls_, ret_type) << " " << fn_name;
        // mark function as external
        func_decls_ << endl << "EXTERNAL " << fn_name << endl;
            ...
            if (param->order() == 0) {
                if (auto ptr = param->type()->isa<PtrType>()) {
                  // check if complex type
                    if (is_complex(ptr->pointee()))
                        func_decls_ << "complex " << param->unique_name() << endl;
                    else if (is_double_complex(ptr->pointee()))
                        func_decls_ << "double complex " << param->unique_name() << endl;
                    else
                    {
                      emit_aggop_decl(ptr->pointee());

                      emit_type(func_decls_, ptr->pointee()) << " " << param->unique_name() << " ! <- Parameter of " << continuation->name() << endl;
                    }
                    insert(param, param->unique_name());
                }
                else
                  assert(false && "All parameters must be call-by-reference.");
            }
        }
    }
    ...
    auto guard = world().name().substr(pos) + ".f95";
    auto name = world().name() + ".f95";
