...
AMDGPUCodeGen::AMDGPUCodeGen(World& world, const Lam2Config& kernel_config)
    : CodeGen(world, llvm::CallingConv::C, llvm::CallingConv::C, llvm::CallingConv::AMDGPU_KERNEL)
    , kernel_config_(kernel_config)
{
    module_->setDataLayout("e-p:64:64-p1:64:64-p2:32:32-p3:32:32-p4:64:64-p5:32:32-p6:32:32-i64:64-v16:16-v24:32-v32:32-v48:64-v96:128-v192:256-v256:256-v512:512-v1024:1024-v2048:2048-n32:64-S32-A5-ni:7");
...
llvm::Function* AMDGPUCodeGen::emit_function_decl(Lam* lam) {
    if (lam->name() == "llvm.amdgcn.implicitarg.ptr")
        if (auto f = thorin::find(fcts_, entry_))
            f->addFnAttr("amdgpu-implicitarg-ptr");
    if (lam->name() == "__ockl_printf_begin")
        if (auto f = thorin::find(fcts_, entry_))
            f->addFnAttr("amdgpu-implicitarg-num-bytes", "32");
    return CodeGen::emit_function_decl(lam);
}
