...
std::unique_ptr<llvm::Module>& CodeGen::emit(int opt, bool debug) {
    ...
    Scope::for_each(world_, [&] (const Scope& scope) {
        ...
        if (debug)
            irbuilder_.SetCurrentDebugLocation(llvm::DebugLoc(llvm::DILocation::get(*context_, entry_->location().front_line(), entry_->location().front_col(), discope)));
        ...
        for (auto& block : schedule) {
            ...
            // terminate bb
            if (debug)
                irbuilder_.SetCurrentDebugLocation(llvm::DebugLoc(llvm::DILocation::get(*context_, continuation->jump_debug().front_line(), continuation->jump_debug().front_col(), discope)));
