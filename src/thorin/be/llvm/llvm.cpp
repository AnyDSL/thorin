...
Lam* CodeGen::emit_reserve_shared(Lam* lam, bool init_undef) {
    ...
    auto elem_type = as<Tag::Ptr>(l->param(1)->type())->arg()->split(0_s)->as<Variadic>()->body();
    ...
    Scope::for_each(world_, [&] (const Scope& scope) {
        ...
        for (auto& block : schedule) {
            ...
            if (lam->app()->callee() == ret_param) { // return
                ...
                auto [cond, a, b] = select->arg()->split<3>();
...
llvm::Value* CodeGen::emit(const Def* def) {
    if (auto iop = isa<Tag::IOp>(def)) {
        auto [a, b] = iop->arg()->split<2>([&](auto def) { return lookup(def); });
        auto name = def->name();
        switch (iop.flags()) {
        ...
        auto [a, b] = wop->arg()->split<2>([&](auto def) { return lookup(def); });
        auto name = def->name();
        auto [mode, width] = wop->decurry()->arg()->split<2>(as_lit<nat_t>);
        ...
        auto name = def->name();
        auto [a, b] = rop->arg()->split<2>([&](auto def) { return lookup(def); });
        auto [mode, width] = rop->decurry()->arg()->split<2>(as_lit<nat_t>);
        ...
        auto [a, b] = icmp->arg()->split<2>([&](auto def) { return lookup(def); });
        auto name = def->name();
        switch (icmp.flags()) {
        ...
        auto [a, b] = rcmp->arg()->split<2>([&](auto def) { return lookup(def); });
        auto name = def->name();
        switch (rcmp.flags()) {
        ...
        auto [num_src, num_dst] = conv->decurry()->arg()->split<2>(as_lit<nat_t>);
        ...
        auto [cond, tval, fval] = select->arg()->split<3>([&](auto def) { return lookup(def); });
