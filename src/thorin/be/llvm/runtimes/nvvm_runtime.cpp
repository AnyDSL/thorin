...
NVVMRuntime::NVVMRuntime(llvm::LLVMContext& context, llvm::Module* target, llvm::IRBuilder<>& builder)
    : KernelRuntime(context, target, builder, llvm::IntegerType::getInt64Ty(context), "nvvm.s")
{}
...
llvm::Value* NVVMRuntime::read(llvm::Value* ptr, llvm::Value* data) {
    auto loaded_device_ptr = builder_.CreateLoad(ptr);
...
llvm::Value* NVVMRuntime::set_problem_size(llvm::Value* x, llvm::Value* y, llvm::Value* z) {
    llvm::Value* problem_size_args[] = { x, y, z };
    return builder_.CreateCall(get("nvvm_set_problem_size"), problem_size_args);
}

llvm::Value* NVVMRuntime::set_config_size(llvm::Value* x, llvm::Value* y, llvm::Value* z) {
    llvm::Value* config_args[] = { x, y, z };
    return builder_.CreateCall(get("nvvm_set_config_size"), config_args);
}

llvm::Value* NVVMRuntime::synchronize() {
    return builder_.CreateCall(get("nvvm_synchronize"));
}

llvm::Value* NVVMRuntime::set_kernel_arg(llvm::Value* ptr) {
    return builder_.CreateCall(get("nvvm_set_kernel_arg"), { ptr });
}

llvm::Value* NVVMRuntime::set_mapped_kernel_arg(llvm::Value* ptr) {
    return builder_.CreateCall(get("nvvm_set_mapped_kernel_arg"), { ptr });
}

llvm::Value* NVVMRuntime::load_kernel(llvm::Value* module, llvm::Value* name) {
    llvm::Value* load_args[] = { module, name };
    return builder_.CreateCall(get("nvvm_load_kernel"), load_args);
}

llvm::Value* NVVMRuntime::launch_kernel(llvm::Value* name) {
