...
llvm::Function* SPIRCodeGen::emit_function_decl(std::string& name, Lambda* lambda) {
    ...
    return current_kernel=f;
}

llvm::Value* SPIRCodeGen::emit_mmap(Def def) {
    assert(current_kernel && "map to shared only in kernel allowed");
    return emit_shared_mmap(def, std::string(current_kernel->getName().str() + "."));
}
