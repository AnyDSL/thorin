...
void SPIRCodeGen::set_data_layout() {
...
Lambda* CodeGen::emit_spir(Lambda* lambda) {
    ...
    builder_.CreateCall(llvm_decls_.get_spir_build_program_and_kernel(), load_args);
    ...
    for (size_t i = 4, e = lambda->num_args(); i < e; ++i) {
        ...
        auto alloca = builder_.CreateAlloca(llvm_decls_.get_spir_device_ptr_type());
        auto device_ptr = builder_.CreateCall(llvm_decls_.get_spir_malloc_buffer(), size);
        // store device ptr
        ...
        builder_.CreateCall(llvm_decls_.get_spir_write_buffer(), mem_args);
        ...
        builder_.CreateCall(llvm_decls_.get_spir_set_kernel_arg(), arg_args);
    }
    ...
    builder_.CreateCall(llvm_decls_.get_spir_set_problem_size(), problem_size_args);
    // launch
    builder_.CreateCall(llvm_decls_.get_spir_launch_kernel(), { kernel_name });
    // synchronize
    builder_.CreateCall(llvm_decls_.get_spir_synchronize());
    ...
    for (size_t i = 4, e = lambda->num_args(); i < e; ++i) {
        ...
        builder_.CreateCall(llvm_decls_.get_spir_read_buffer(), args);
    }

    // free memory
    for (auto device_ptr : device_ptrs)
        builder_.CreateCall(llvm_decls_.get_spir_free_buffer(), { device_ptr.first });
    return ret;
}
