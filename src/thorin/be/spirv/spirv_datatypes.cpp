...
ConvertedType& CodeGen::convert(const Type* type) {
    if (auto iter = types_.find(type); iter != types_.end()) return iter->second;

    assert(!type->isa<MemType>());
    ConvertedType& converted = types_.emplace(type, ConvertedType {this } ).first->second;
    switch (type->tag()) {
        ...
        case PrimType_bool:
            ...
            converted.datatype = std::make_unique<ScalarDatatype>(converted, type->tag(), 1, 4);
            break;
