...
const Def* Lam::mem_param(thorin::Dbg dbg) {
    return thorin::isa<Tag::Mem>(param(0)->type()) ? param(0, dbg) : nullptr;
}

const Def* Lam::ret_param(thorin::Dbg dbg) {
    if (num_params() > 0) {
...
void Lam::branch(const Def* cond, const Def* t, const Def* f, const Def* mem, Dbg dbg) {
    return app(world().extract(world().tuple({f, t}), cond, dbg), mem, dbg);
}

void Lam::match(const Def* val, Defs cases, const Def* mem, Dbg dbg) {
