...
Param::Param(const Type* type, Lam* lambda, size_t index, Debug dbg)
...
void Lam::match(const Def* val, Lam* otherwise, Defs patterns, ArrayRef<Lam*> lambdas, Debug dbg) {
    Array<const Def*> args(patterns.size() + 2);

    args[0] = val;
    args[1] = otherwise;
    assert(patterns.size() == lambdas.size());
    for (size_t i = 0; i < patterns.size(); i++)
        args[i + 2] = world().tuple({patterns[i], lambdas[i]}, dbg);
...
void Lam::verify() const {
    if (!has_body()) {
        assertf(filter()->is_empty(), "lambdas with no body should have an empty (no) filter");

        if (world().is_external(this)) {} // external (imported) lambdas may have no body
        else if (dead_) {}
...
void jump_to_dropped_call(Lam* lambda, Lam* dropped, const Defs specialized_args) {
    assert(lambda->has_body());
    auto obody = lambda->body();
...
bool visit_capturing_intrinsics(Lam* lam, std::function<bool(Lam*)> func, bool include_globals) {
    return visit_uses(lam, [&] (auto use) {
        if (!use->has_body()) return false;
        auto body = use->body();
...
bool is_passed_to_accelerator(Lam* lam, bool include_globals) {
    return visit_capturing_intrinsics(lam, [&] (Lam* capturing_lam) { return capturing_lam->is_accelerator(); }, include_globals);
}

bool is_passed_to_intrinsic(Lam* lam, Intrinsic intrinsic, bool include_globals) {
    return visit_capturing_intrinsics(lam, [&] (Lam* capturing_lam) { return capturing_lam->intrinsic() == intrinsic; }, include_globals);
}
