...
const Def* PtrnCompiler::rewrite(Def*, const Def* def) {
    if (auto match = def->isa<Match>()) return compile(match);
    return def;
}

Ptrn* PtrnCompiler::flatten(Ptrn* ptrn) {
    auto f_ptrn = world().ptrn(world().case_(thorin::flatten(ptrn->type()->domain()), ptrn->type()->codomain()), ptrn->debug());
...
std::pair<const Def*, const Def*> PtrnCompiler::eliminate(const Def* def, size_t col) {
    ...
    for (size_t i = col, n = ops.size(); i < n; ++i)
        ops[i] = def->out(i + 1);
    return { def->out(col), world().tuple(ops) };
}

const Def* PtrnCompiler::introduce(const Def* def, size_t col, const Def* val) {
    // See above.
    auto type = def->type()->reduce();
    if (!type->isa<Sigma>())
        return world().tuple({col == 0 ? val : def, col == 0 ? def : val});
    if (type->num_ops() == 0)
        return val;
    Array<const Def*> ops(type->num_ops() + 1);
    for (size_t i = 0; i < col; ++i)
...
Ptrn* PtrnCompiler::specialize(Ptrn* ptrn, size_t col, const Def* ctor, const Def* arg_col, const Def* s_arg, bool d_arg_was_empty) {
    ...
    auto s_ptrn = world().ptrn(world().case_(s_arg->type(), ptrn->body()->type()), ptrn->debug());
    const Def* param = ptrn->param(), *s_param = s_ptrn->param();
    if (arg_col) {
        // HACK: Handle the case where we have T and we want E::A(T)
        if (d_arg_was_empty) {
            s_param = world().insert(world().bot(arg_col->type()), ctor, s_param);
        } else {
            // We have (T, ...) and we want (E::A(T), ...)
            const Def* s_param_col = nullptr;
            std::tie(s_param_col, s_param) = eliminate(s_param, col);
            s_param = introduce(s_param, col, world().insert(world().bot(arg_col->type()), ctor, s_param_col));
        }
    } else {
...
bool PtrnCompiler::is_complete(const Def* arg_type, const DefMap<std::vector<Ptrn*>>& ctor2ptrns) {
    if (auto lit = isa_lit<nat_t>(arg_type))
        return ctor2ptrns.size() == *lit;
    return false;
}

const Def* PtrnCompiler::compile(const Match* match, const Def* arg, std::vector<Ptrn*>& ptrns, const Def* dbg) {
    assert(!ptrns.empty());
    ...
    auto ctor = has_union ? world().which(arg_col) : arg_col;
    auto ctor_type = ctor->type();
    bool d_arg_was_empty = d_arg->type()->reduce() == world().sigma();
    ...
    for (auto& [ctor, ctor_ptrns] : ctor2ptrns) {
        ...
        if (has_union)
            s_arg = introduce(s_arg, col, world().extract(arg_col, ctor));
        ...
        auto ptrn = world().ptrn(world().case_(ctor_type, value->type()), dbg);
        ptrn->set(ctor, value);
        compiled_ptrns.push_back(ptrn);
    }
    if (!is_complete(ctor_type, ctor2ptrns)) {
        if (no_ctor.empty()) {
            if (world().err()) world().err()->incomplete_match(match);
            return world().bot(match->type());
        }
        for (auto& ptrn : no_ctor)
    ...
    return world().match(ctor, compiled_ptrns, dbg);
}

const Def* PtrnCompiler::compile(const Match* match) {
    ...
    if (world().err()) {
        for (auto ptrn : match->ptrns()) {
            if (redundant_[ptrn])
                world().err()->redundant_match_case(match, ptrn->as<Ptrn>());
        }
    }
    return result;
}
