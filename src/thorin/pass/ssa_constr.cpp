...
std::variant<const Def*, undo_t> SSAConstr::rewrite(Def* cur_nom, const Def* def) {
    ...
    } else if (def->isa<Param>() || def->isa<Proxy>()) {
        return def;
    }
    ...
    for (size_t i = 0, e = def->num_ops(); i != e; ++i) {
        if (auto lam = def->op(i)->isa_nominal<Lam>()) {
            if (keep_.contains(lam)) continue;

            auto callee_pos = app != nullptr && i == 0;
            ...
            if (preds_n_.contains(lam)) {
                ...
                if (inserted) {
                    ...
                    visit.pred = cur_lam;
                    world().DLOG("Preds1_callee_pos{} {} with pred {}", callee_pos, lam, visit.pred);
                } else {
                    if (visit.callee_pos && callee_pos) {
                        ...
                        world().DLOG("Preds1_callee_pos_{}_{} -> PredsN: {}", visit.callee_pos, callee_pos, lam);
                        keep_.emplace(lam);
                    }
                    return undo;
                }
...
std::variant<const Def*, undo_t> SSAConstr::rewrite(Lam* cur_lam, const App* app, Lam* mem_lam) {
    ...
    if (auto& phis = lam2phis_[mem_lam]; !phis.empty()) {
        ...
        if (phi_lam == nullptr) {
            ...
            size_t i = 0;
            const Def* stoxy = phi_lam->param();
            for (auto phi : phis)
                stoxy = proxy(stoxy->type(), {stoxy, phi, phi_lam->param(n + i++)}, Stoxy, phi->debug());

            Array<const Def*> new_params(mem_lam->num_params(), [&](size_t i) { return phi_lam->param(i); });
