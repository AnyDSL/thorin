            ...
            if (auto ucontinuation = use->isa_continuation()) {
                if (auto callee = ucontinuation->callee()->isa_continuation()) {
                    if (callee->is_intrinsic()) {
                        ...
                        new_ops[use.index()] = world.global(lifted, lifted->location(), false, lifted->name);               // update to new lifted continuation
                        ucontinuation->jump(cur, new_ops.skip_front(), ucontinuation->jump_location());                     // set new args

                        // jump to new top-level dummy function
                        auto ncontinuation = world.continuation(ucontinuation->arg_fn_type(), callee->location(), callee->cc(), callee->intrinsic(), callee->name);
