...
Continuation* Mangler::mangle() {
    ...
    new_entry_ = world().continuation(fn_type, old_entry()->location(), old_entry()->name);
...
void Mangler::mangle_body(Continuation* old_continuation, Continuation* new_continuation) {
    ...
    if (old_continuation->callee() == world().branch()) {        // fold branch if possible
        if (auto lit = mangle(old_continuation->arg(0))->isa<PrimLit>())
            return new_continuation->jump(mangle(lit->value().get_bool() ? old_continuation->arg(1) : old_continuation->arg(2)), {}, old_continuation->jump_location());
    }
    ...
    if (ntarget == old_entry()) {
        ...
        if (substitute) {
            ...
            return new_continuation->jump(new_entry(), args, old_continuation->jump_location());
        }
    }

    new_continuation->jump(ntarget, nargs, old_continuation->jump_location());
}
